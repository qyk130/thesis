...
Dota2 is one of the most popular online game. It is also the originator of multiplayer online battle arena(MOBA) game. Professional Dota2 tournaments often offer prizes in millions of US dollars in total. There are over 6 millions of players who have played at least one match in February 2017. The popularity also brought a huge amount of data on players and matches. Valve, the developer of Dota2 produces API(Application Programming Interface) for people to access these data.  \\

In a regular game of Dota, there are ten players separated to two sides. Each player can pick one of 113 heroes in the match. Players often play in different roles by convention. Each hero has different attribute and unique abilities which decide what roles it is capable of. When players pick heroes they need to consider the combination of roles and heroes, some powerful hero combos and countering enemies' pick. This is not easy because the game is extremely variable. For example: one hero can often be played as different roles; a certain item may increase a hero's strength or counter an enemy hero dramatically; the relationship between heroes: synergy or counter varies with time.\\
...
Machine learning learns patterns from a sample to solve problems. It has a variety of uses include credit applications, fraud detection, medical diagnosis, spam filters and many other applications\cite{alpaydin2014introduction}. There are different models for different types of problems. In this case, we have a classification problem. Logistic Regression and Naive Bayes are proposed to solve this problem due to their simplicity and efficiency.

\chapter{Literature review / prior art}

Dota2 has drawn a fair amount of attention of machine learning researchers in the last few years due to its complexity and popularity. \\
	
In 2013 Conley and Perry presented a hero recommendation engine depends on the opposite draft\cite{conley2013does}. They performed Logistic Regression and K-Nearest Neighbors on the draft information of a certain amount of matches. They achieved 69.8\% accuracy with Logistic Regression on 18,000 matches, however, it couldn't learn from the relationships between heroes. To attempt solving that issue they used K-Nearest Neighbors with custom weights for neighbors with 2-fold cross-validation on 20,000 matches. Their best accuracy with 4 dimensions got 67.43\% accuracy. The recommendation engine was built on these approaches. Their work was the first to use the draft information to predict Dota2 games result, but a linear model like Logistic Regression or simple model like K-Nearest Neighbors can not capture the interaction between heroes.\\

In 2014 Agarwala and Pearce studied how win rate depends on the draft by performing Logistic Regression with the draft directly, second order polynomial of PCA scores and PCA sorted by the first PCA component respectively on 1500 matches \cite{agarwala2014learning}. They selected some features to describe a hero's role by normalizing the average hero statistics. Then PCA was run on the average statistics of professional play. The result indicated that the PCA with draft models failed to match the accuracy of the pure draft model. The authors hypothesized that average hero stats are not so informative since one hero can play different roles. Their PCA score was based on professional play but they trained on public matches. Professional matches and public matches are very different and shouldn't be mixed, which should be another reason.\\

In 2015 Song et al.\cite{song2015predicting}. tried to use Logistic Regression to predict the winning side of Dota2 based on the draft. After discovering that only draft information is not enough to train a precise model, they attempted to add features based on powerful 2-hero combos using stepwise regression and the result was slightly better. They mentioned that the information of player should increase the model's accuracy in the conclusion.\\

 In the same year Kinkade et al.\cite{kinkade2015dota} presented two classifiers to predicts the winning team of Dota2. One uses post game data, which performed perfectly but does not hold any real use. The other one uses pre-game draft information. The authors used Logistic Regression with several manually inserted features such as matchup, synergy and countering between heroes and they used Random Forest with only draft information. Logistic Regression got only 56\% accuracy with draft information itself and the features increases the model's quality to up to 73\%. For random forest, the authors found that overfitting was a large issue. They got 99\% accuracy on the training set while they only got 67\% accuracy on the testing set after several parameters verified. Logistic Regression performed significantly better. The authors hypothesized that the players' skill contributes almost linearly to the game so a linear model would fit better.\\

In 2016 Yang et al.\cite{yang2016real} predict the winning team of a Dota2 match using prior and real-time data. The authors constructed a feature set covers more aspects including hero, player and hero-player combination as prior features and other real-time match stats as real-time features. They used Logistic Regression, neural network and the proposed Attribute Sequence Model and their combinations on prior features, and both prior and real-time features. For the prior game prediction, the result shows that using all features achieves the highest prediction accuracy 71.49\%. For the real-time prediction, the results show that prior features' impact on the prediction drops over time while the real-time's increases. The idea of using the combination of information of hero and player is impressive. \\
